...
const displayPopularMovies = async () => {
  ...
  results.forEach(movie => {
    const { title, release_date, id, poster_path } = movie;
    ...
    <p class="card-text"><small class="text-muted"> Release: ${release_date}</small></p>
    </div>
    `;
...
const displayPopularShows = async () => {
  ...
  results.forEach(shows => {
    const { name, first_air_date, id, poster_path } = shows;
    const div = document.createElement('div');
    div.classList.add('card');
    div.innerHTML = `
    <a href="movie-details.html?id=${id}">
    ...
    <p class="card-text"><small class="text-muted"> Air Date: ${first_air_date}</small></p>
    </div>
    `;
...
const fetchAPIData = async (endpoint) => {
  showSpinner();
  const API_KEY = '586c52031e54e4636ae66fa6d251246c';
